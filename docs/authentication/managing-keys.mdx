...
<Callout type="info">
  **Subscription Required:** You must have an active developer subscription
  (Free or paid) before creating API keys. The API key will match your current
  plan.
</Callout>
...
<Steps>
  <Step>
    ...
    If you don't have an active subscription, go to [Upgrade](https://developers.brainus.lk/upgrade) and subscribe to a Free or paid plan. No credit card required for the Free plan!
  </Step>

{" "}
<Step>
  ### Navigate to Dashboard Go to [Dashboard → API
  Keys](https://developers.brainus.lk/dashboard/api-keys)
</Step>

{" "}

<Step>### Generate New Key Click **"Create API Key"** button</Step>
...
<Callout type="warn">
  If you don't have an active subscription, the "Create API Key" button will be
  disabled. Subscribe to a plan first at
  [Upgrade](https://developers.brainus.lk/upgrade).
</Callout>
